-- 20260812000000_init.sql
-- Create database schema for QR Club Attendance System

-- Timezone
-- Note: PostgreSQL supports 'Asia/Kolkata' timezone offsets globally.
...
CREATE POLICY "Admins can view all admin user roles"
  ON public.admin_users FOR SELECT
  USING (public.is_admin());

CREATE POLICY "Presidents can manage admin user roles"
...
DROP POLICY IF EXISTS "Public can view basic metadata for QR token confirmation only" ON public.club_members;
CREATE POLICY "Admins full select on club members"
...
CREATE POLICY "Public can view events"
  ...
  USING (true); -- Public needs to see event name/date for fallback check-in.

-- Scoring Rules Policies
CREATE POLICY "Admins can manage scoring rules"
  ON public.event_scoring_rules FOR ALL
  USING (public.is_admin());

CREATE POLICY "Public can view scoring rules"
  ON public.event_scoring_rules FOR SELECT
  USING (true);
...
CREATE POLICY "Public can insert attendance (for self check-in)"
  ON public.attendance FOR INSERT
  WITH CHECK (true); -- Checked and verified in RPC.

CREATE POLICY "Public can view attendance (limited)"
  ON public.attendance FOR SELECT
  USING (public.is_admin()); -- Only admins can select.

-- Attendance Audit Log Policies
CREATE POLICY "Admins can manage attendance audit logs"
...
RETURNS VOID AS $$
BEGIN
  -- Clear existing rules for the event
  DELETE FROM public.event_scoring_rules WHERE event_id = p_event_id;

  -- 21+ minutes early -> 130
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'early', -9999, -21, 130, '21+ min early');

  -- 11-20 minutes early -> 115
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'early', -20, -11, 115, '11-20 min early');

  -- 5-10 minutes early -> 110
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'early', -10, -5, 110, '5-10 min early');

  -- 0-5 minutes after start -> 100 (also covers up to 4 minutes early, let's start from -4)
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'on_time', -4, 5, 100, 'On Time (0-5 min)');

  -- 6-10 minutes late -> 95
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 6, 10, 95, '6-10 min late');

  -- 11-15 minutes late -> 90
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 11, 15, 90, '11-15 min late');

  -- 16-25 minutes late -> 80
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 16, 25, 80, '16-25 min late');

  -- 26-30 minutes late -> 60
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 26, 30, 60, '26-30 min late');

  -- 31-45 minutes late -> 40
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 31, 45, 40, '31-45 min late');

  -- 46-58 minutes late -> 10
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 46, 58, 10, '46-58 min late');

  -- 59+ minutes late -> 0
  INSERT INTO public.event_scoring_rules (event_id, rule_type, min_minutes, max_minutes, score, label)
  VALUES (p_event_id, 'late', 59, 9999, 0, '59+ min late');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Checks for events on the given date (or around it) that are scheduled/active.
-- Active is defined as: server date/time is within event start_time and end_time, or state is 'active'.
...
BEGIN
  ...
  WHERE 
    ...
    AND p_now >= (e.start_time - INTERVAL '2 hours') -- Allow scans up to 2 hours early
    AND p_now <= (COALESCE(e.attendance_cutoff_time, e.end_time) + INTERVAL '1 hour') -- Allow scans up to 1 hour after cutoff/end
  ORDER BY 
...
-- Admin scans a physical ID card (p_member_qr_token).
-- Resolves member, checks if active, automatically resolves the active event, calculates score, records attendance, and creates audit log.
...
BEGIN
  ...
  ELSE
    -- Count active events in time frame
    SELECT COUNT(*), MAX(e.id)
    INTO v_active_event_count, v_event_id
    FROM public.events e
  ...
  ELSIF v_active_event_count > 1 THEN
    -- If multiple overlapping events exist, raise warning or return details
  ...
  SELECT e.name, e.attendance_cutoff_time, e.end_time
  INTO v_event_name, v_cutoff, v_end
  FROM public.events e
  ...
  IF EXISTS (SELECT 1 FROM public.attendance WHERE event_id = v_event_id AND member_id = v_member_id) THEN
    SELECT a.scanned_at, a.score, a.timing_category, a.minutes_from_start
    INTO scan_time, v_pts, v_timing_cat, v_m_from_start
    FROM public.attendance a
    WHERE a.event_id = v_event_id AND a.member_id = v_member_id;
  ...
  VALUES (v_event_id, v_member_id, p_now, v_pts, v_timing_cat, v_m_from_start, 'id_card_scan')
  ...
  -- Update event status to active if it was scheduled
...
-- Member performs self check-in by scanning event QR code token, then submitting Name + Position.
...
DECLARE
  ...
  v_m_from_start INTEGER;
BEGIN
  -- 1. Validate the event QR token
  SELECT e.id, e.name, e.status, e.attendance_cutoff_time, e.end_time
  INTO v_event_id, v_event_name, v_event_status, v_cutoff, v_end
  FROM public.events e
  WHERE e.event_qr_token = p_event_qr_token;
  ...
  -- 2. Validate member Name + Position
  -- Case-insensitive exact name and position matching
  SELECT cm.id, cm.full_name, cm.position, cm.is_active
  INTO v_member_id, v_member_name, v_member_position, v_is_active
  FROM public.club_members cm
  ...
  -- 5. Record attendance
  INSERT INTO public.attendance (event_id, member_id, scanned_at, score, timing_category, minutes_from_start, attendance_method)
  VALUES (v_event_id, v_member_id, p_now, v_pts, v_timing_cat, v_m_from_start, 'self_checkin')
  RETURNING id INTO v_attendance_id;

  -- 6. Write audit log (scanned_by = NULL for self check-in)
  INSERT INTO public.attendance_audit_log (attendance_id, event_id, member_id, scanned_by, attendance_method, scanned_at)
...
BEGIN
  ...
  SET qr_status = 'revoked', qr_revoked_at = now(), updated_at = now()
...
BEGIN
  ...
  UPDATE public.club_members
  SET qr_status = 'active', qr_created_at = now(), qr_revoked_at = NULL, member_qr_token = v_new_token, updated_at = now()
  WHERE id = p_member_id;
...
BEGIN
  ...
  -- Get total completed/active events in this month
  ...
  IF v_total_events = 0 THEN
    v_total_events := 1; -- Avoid division by zero
  END IF;
