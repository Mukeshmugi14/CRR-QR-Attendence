...
import { Trophy, Award, Target, Percent, UserCheck } from 'lucide-react';
...
export default async function AllTimeLeaderboardPage() {
  ...
  // Query All-Time Leaderboard via RPC
  const { data: leaderboard, error } = await supabase.rpc('get_all_time_leaderboard');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Browse overall club statistics, highest individual records, and historical points accumulations.
          </p>
        </div>
      </div>
        ...
        {leaderboard && leaderboard.length > 0 ? (
          leaderboard.map((row: any) => (
            <div key={row.member_id} className="p-4 rounded-2xl bg-slate-900 border border-slate-800">
              ...
              <div className="mt-3 pt-3 border-t border-slate-850 grid grid-cols-4 gap-2 text-center">
                <div>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-semibold mt-0.5">Avg</p>
                </div>
                <div>
                  <div className="flex items-center justify-center gap-1 text-slate-200 text-sm font-bold">
                    <Target className="h-3.5 w-3.5 text-indigo-400" />
                    {Number(row.best_event_score)}
                  </div>
                  <p className="text-[10px] text-slate-500 font-semibold mt-0.5">Best</p>
                </div>
                <div>
                  <div className="flex items-center justify-center gap-1 text-slate-200 text-sm font-bold">
                  ...
                  <p className="text-[10px] text-slate-500 font-semibold mt-0.5">Rate</p>
                </div>
              </div>
            </div>
          ))
          ...
          <div className="text-center py-12 rounded-2xl bg-slate-900 border border-slate-800 text-slate-500 text-sm px-4">
            No events held or member scores recorded yet.
          </div>
        )}
      </div>
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="border-b border-slate-800 text-slate-400 text-xs font-bold uppercase tracking-wider">
                  ...
                  <th className="py-3 px-4 text-center">Best Event Score</th>
