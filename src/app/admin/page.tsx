...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-slate-800">
        <div>
          <h1 className="text-2xl font-extrabold text-white sm:text-3xl">Club Operations Dashboard</h1>
          <p className="text-sm text-slate-400 mt-1">Manage events, scan club ID cards, view leaderboards, and print reports.</p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <Link
            href="/admin/scanner"
            className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 active:scale-95 transition-all"
          >
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.name} className="p-5 rounded-2xl bg-slate-900 border border-slate-800/80 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-400">{stat.name}</p>
                <p className="mt-2 text-2xl font-extrabold text-slate-100">{stat.value}</p>
              </div>
      ...
      {activeEvent && (
        <div className="p-6 rounded-2xl bg-slate-900 border-2 border-indigo-500/30 shadow-xl shadow-indigo-500/5 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4 text-xs text-slate-400 pt-2">
                <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {new Date(activeEvent.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(activeEvent.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                <span>•</span>
                <span>Date: {new Date(activeEvent.event_date).toLocaleDateString([], { day: 'numeric', month: 'short', year: 'numeric' })}</span>
              </div>
            </div>
            
            <div className="flex items-center gap-8 bg-slate-950/40 p-4 rounded-xl border border-slate-800/80 shrink-0">
              <div className="text-center">
                <p className="text-2xl font-black text-slate-100">{Number(activeEvent.present_count)} / {Number(activeEvent.total_members)}</p>
                <p className="text-xs text-slate-500 font-semibold mt-1">Present Members</p>
              </div>
              <div className="h-10 w-px bg-slate-800" />
              <div className="text-center">
                <div className="flex items-center gap-1 justify-center">
                  <Percent className="h-4 w-4 text-indigo-400" />
                  <p className="text-2xl font-black text-slate-100">{attendancePercentage}%</p>
                </div>
            ...
            {recentEvents && recentEvents.length > 0 ? (
              <div className="space-y-3">
                {recentEvents.map((evt) => (
                  <div key={evt.id} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-950/40 border border-slate-850 hover:border-slate-800 transition-colors">
                    <div>
                      <h4 className="text-sm font-bold text-slate-200">{evt.name}</h4>
